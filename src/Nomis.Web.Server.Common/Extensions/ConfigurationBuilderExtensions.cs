...
namespace Nomis.Web.Server.Common.Extensions
{
    ...
    public static class ConfigurationBuilderExtensions
    {
        ...
        public static IConfigurationBuilder AddJsonConfigs(this IConfigurationBuilder manager)
        {
            ...
            return manager
                .AddJsonFile(Path.Combine(ConfigsDirectory, $"{nameof(BscscanSettings).ToLower()}.json"), false, true)
                .AddJsonFile(Path.Combine(ConfigsDirectory, $"{nameof(BscscanSettings).ToLower()}.{env}.json"), true, true);
        }
    }
}